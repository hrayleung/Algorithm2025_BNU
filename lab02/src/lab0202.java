...
public class lab0202 {
    ...
    private static int[][] strassenMultiply(int[][] a, int[][] b, int n) {
        ...
        int[][] m1 = strassenMultiply(addMatrix(a11, a22, newSize),
                addMatrix(b11, b22, newSize), newSize);
        int[][] m2 = strassenMultiply(addMatrix(a21, a22, newSize),
                b11, newSize);
        int[][] m3 = strassenMultiply(a11,
                subtractMatrix(b12, b22, newSize), newSize);
        int[][] m4 = strassenMultiply(a22,
                subtractMatrix(b21, b11, newSize), newSize);
        int[][] m5 = strassenMultiply(addMatrix(a11, a12, newSize),
                b22, newSize);
        int[][] m6 = strassenMultiply(subtractMatrix(a21, a11, newSize),
                addMatrix(b11, b12, newSize), newSize);
        int[][] m7 = strassenMultiply(subtractMatrix(a12, a22, newSize),
                addMatrix(b21, b22, newSize), newSize);

        int[][] res11 = addMatrix(
                subtractMatrix(
                        addMatrix(m1, m4, newSize),
                        m5, newSize),
                m7, newSize);
    ...
    private static void splitMatrix(int[][] original,
                                    int[][] a11, int[][] a12,
                                    int[][] a21, int[][] a22, int size) {
    ...
    private static void joinMatrix(int[][] res,
                                   int[][] a11, int[][] a12,
                                   int[][] a21, int[][] a22, int size) {
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
